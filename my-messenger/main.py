...
# Хелпер для исправления ошибки ObjectId
def clean_id(data):
...
def get_user_data(uid):
    return jsonify(clean_id(db.users.find_one({"_id": ObjectId(uid)}, {"pw": 0})))
...
def manage_groups():
    ...
    if request.method == 'DELETE':
        ...
        if g and g['owner'] == str(u['_id']):
            ...
            return "ok"
    return jsonify(clean_id(list(db.groups.find({"members": str(u['_id'])}))))
...
def get_history(rid):
    return jsonify(clean_id(list(db.messages.find({"room": rid}).sort("ts", -1).limit(50)))[::-1])
